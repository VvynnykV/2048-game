...
button.addEventListener("click", ()=>{
    if (button.classList.contains("restart")) game.restart();
    game.start();
    updateUi();
...
function updateUi() {
    ...
    if (curStatus === "lose") loseMessage.classList.remove("hidden");
    else if (curStatus === "win") winMessage.classList.remove("hidden");
}
...
class Game {
    ...
    findEmptyCell() {
        ...
        for(let row = 0; row < BOARD_SIZE; row++){
            for(let col = 0; col < BOARD_SIZE; col++)if (this.state[row][col] === 0) emptyCells.push([
                row,
                col
            ]);
        }
    ...
    isGameOver() {
        ...
        const canMergeCells = this.state.some((row, rowIndex)=>{
            return row.some((cell, cellIndex)=>{
                ...
                if (cellIndex < BOARD_SIZE - 1 && cell === row[cellIndex + 1]) return true;
                // Check down neighbor
                if (rowIndex < BOARD_SIZE - 1 && cell === this.state[rowIndex + 1][cellIndex]) return true;
                return false;
            });
        });
    ...
    updateGameStatus() {
        if (this.hasWinCell()) this.status = "win";
    ...
    areStatesChanged(state1, state2) {
        return state1.every((row, rowIndex)=>{
            return row.every((cell, cellIndex)=>cell === state2[rowIndex][cellIndex]);
        });
    }
}
module.exports = Game;
